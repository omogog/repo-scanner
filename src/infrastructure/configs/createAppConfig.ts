...
export const createAppConfig = () => {
  ...
  const maxConcurrentRequests = parseInt(<string>process.env.MAX_CONCURRENT_REQUESTS || "2", 10);

  const gitClient = new GitHubClient(baseURL);
  const repositorySummaryCache = new InMemoryCache<RepositorySummary[]>(cacheTTL);
  ...
  const listRepositories = new ListRepositories(gitClient, repositorySummaryCache);
  const getRepositoryDetails = new GetRepositoryDetails(gitClient, repositoryDetailCache, queue);
