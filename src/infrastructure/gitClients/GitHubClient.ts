...
export class GitHubClient implements GitClient {
  ...
  async getRepositoryDetails(
    ...
    const activeWebhooks = await this.fetchRepositoryWebhooks(
      config,
      userLogin,
      repoName
    );
  ...
  private async fetchUserLogin(config: any): Promise<string> {
    const response = await this.httpClient.get<UserData>(
      `${this.baseURL}/user`,
      config
    );
  ...
  private async fetchRepositoryData(
    ...
    return this.httpClient.get<RepoData>(
      `${this.baseURL}/repos/${userLogin}/${repoName}`,
      config
    );
  }
  ...
  private async fetchRepositoryContents(
    ...
    return this.httpClient.get<ContentFile[]>(
      `${this.baseURL}/repos/${userLogin}/${repoName}/contents`,
      config
    );
  }

  private async fetchRepositoryWebhooks(
    config: any,
    userLogin: string,
    repoName: string
  ): Promise<string[]> {
    try {
      const response = await this.httpClient.get<Webhook[]>(
        `${this.baseURL}/repos/${userLogin}/${repoName}/hooks`,
        config
      );
  ...
  private async fetchYmlFileContent(
    files: ContentFile[]
